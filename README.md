...
## QuickSay是干什么的？
不知道你会不会把常用短语保存在一个txt文件里，并在需要用到时打开这个txt文件复制粘贴。  
那么你就可以把常用短语保存在这个软件里，并使用标签分类。  
...
4. 在文件资源管理器（就是此电脑）里找到这个新建的项目，把第1步下载下来的 QuickSay文件夹 直接移动到这个项目文件夹里，选“替换目标中的文件”  
