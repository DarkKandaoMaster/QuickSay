...
#### 方法一：
看到右边的发行版（Releases）了吗？点它→，于是进入项目发布页，再点击QuickSay_v1.1.0.7z，于是开始下载  
下载完成后，将得到的文件解压，于是得到  
```
QuickSay文件夹
...
然后点击QuickSay_v1.1.0.7z，再点下载按钮，于是开始下载  
接下来的步骤同上  
...
- 适合Qt新手自学。代码注释真的超级超级详细，代码内容也比较简单，完全不用像学算法那样理解半天都理解不了这段代码是干什么的。如果把这些代码给曾经的我看的话，估计用不了一周我就能自学成才，并且发布软件了  
...
- 还有就是UI。如果你们觉得UI有什么不合适的地方欢迎来提建议。我是觉得UI设置成这样已经很好看了，但完不完美我不知道  
...
- 可以同时打开多个添加窗口、多个修改窗口、多个语录快捷键窗口。诶问下这个功能你们想要吗？想要的话我尽力实现，但不知道能不能实现出来...  
- 快捷键输入框允许输入多个快捷键的组合，但就算这么输入，也应用不了。所以我是通过把它判定为不合规快捷键的方式来阻止用户这么输入的。但其实还有个思路就是检测到用户输入主键后立刻清除快捷键输入框的焦点。但目前我能力不够...别看它看起来简单，实现起来可麻烦了  
- 用户可以自定义多个标签，点击不同标签显示不同语录。就是一个对语录进行分类的功能。唔这个功能想想都麻烦，要是实现它的话估计又是一次大改动...  
...
最小能用版本只需要277行，提升用户体验需要1161行（笑哭）...  
...
唉想念我的暑假了...这几天经常更新到一半就要去上课了。暑假你不要走了好不好...  
...
OK！于是QuickSay就一鼓作气地更到了v1.1.1。可以说现在功能已经比较完善了。  
没想到今天还是CSP初赛。祝大家加油呀！  
