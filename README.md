...
#### 方法一：
看到右边的发行版（Releases）了吗？点它→，于是进入项目发布页，再点击QuickSay_v1.0.1.7z，于是开始下载  
下载完成后，将得到的文件解压，于是得到  
```
QuickSay文件夹
QuickSay全部使用操作.txt
如何安装或更新QuickSay.txt
```
直接把 **QuickSay文件夹** 放到随便什么路径下（有中文空格都行）（但是不要把该软件放到Program Files以及它的子文件夹，这种文件夹好麻烦的改个语录都需要管理员权限），然后双击打开里面的exe文件就可以正常运行软件了。  
那两个txt文件懒得看的话可以直接删除，没关系的（但其实还是建议看一下...）  
#### 方法二：
直接点进这个蓝奏云链接：https://wwlt.lanzoum.com/iJmko34w5rre 密码:3w9p  
然后点下载按钮，于是开始下载  
接下来的步骤同上  


## 如何更新
下载新版的压缩包之后，将得到的文件解压，然后点进QuickSay文件夹，删除里面的 **data.json** 和 **config.json**  
然后用解压出来的QuickSay文件夹**替换**掉原来的QuickSay文件夹就行了。  
...
对了这个软件不需要联网的，所有数据都保存在本地。  
因为是自己开发的，所以免费、开源、无广、无需登录，未来我也不打算加上这些东西。总之放心使用吧！  

这里放一张软件窗口的全家福↓  
<img src="./README_Pictures/软件窗口的全家福.png" alt="图片无法显示。是真的无法显示，估计是你的网络问题">
...
- 如果你在添加/修改/删除语录，或者改变语录顺序后，退出并重新打开QuickSay，发现语录根本没被修改，还是原来的语录，那这必然是因为你把软件安装在Program Files以及它的子文件夹，都说了别把软件放在这种文件夹里了...  
  × 解决方法：把软件放在其他文件夹里，然后重新打开QuickSay.exe  
...
6. 于是就可以运行了！点击左侧的那个绿色三角形，您会发现弹出了个和QuickSay发行版一模一样的窗口。那么恭喜您！此时您就可以正常运行代码了。**与我一同，成为开发者吧！**  
##### 说几个容易踩的坑：
...
我去...这里记录一个我改bug的心理历程：  
v1.0.0发布当天，我就发现了个bug：在微信电脑版的输入框里使用QuickSay居然输入不进去。于是我立刻进行研究。  
结果越研究我越像是被浇了盆冷水，直到研究到PowerToys，我几乎完全绝望了：居然连PowerToys的高级粘贴都做不到在微信电脑版输入框里粘贴...那这个bug我好像是真的无能为力。毕竟连PowerToys都做不到的事，我一个萌新，一个平凡得不能再平凡的普通人，又怎么做得到？  
我似乎能理解为什么市面上好用的电脑版快捷语录软件几乎没有了。于是此时我的新README文件都已经写好了：“我去...了解了一下，这个bug我好像是真的无能为力...毕竟连PowerToys的高级粘贴都做不到在微信电脑版输入框里粘贴...那我这个小软件就更不用说了...”  
但我又想了想，毕竟很多人的聊天区域都在微信，要是我就这么把软件发出去，感觉没法给用户，给QuickSay一个交代。于是我没有把写好的新README文件立刻传上去，而是继续研究。  
为什么Windows剪贴板历史记录能够在电脑版输入框里输入，而PowerToys的高级粘贴却不能？我很幸运，最终我想到了延时输出。  
体感上Windows剪贴板历史记录输出的时候会慢很多毫秒，在这之前我一直都以为这是因为它太差了所以卡了一下，但经过今天这么一对比，我才知道这可能是Windows剪贴板历史记录能够在任何地方正常输出的秘密。  
于是我就在shuchu函数里加了个singleShot，让它延时个50毫秒再输出。于是我成功了！QuickSay成功在微信电脑版的输入框里输入了！  
我靠那时给我激动的...于是我啊啊啊叫了半天。  
所以这里也给PowerToys的高级粘贴一个建议：延时个50毫秒再粘贴。  
...
<img src="./README_Pictures/碎碎念1.png" alt="图片无法显示。是真的无法显示，估计是你的网络问题" width="300"> <img src="./README_Pictures/碎碎念2.png" alt="图片无法显示。是真的无法显示，估计是你的网络问题">
我已经百刀不侵啦！（戴墨镜）  
唉果然还是有点遗憾呀...  
...
哈哈，话说我第一次推魔宴甚至还推进了共通线end。单身结局，不愧是我。  

其实本来启动QuickSay这个项目之前我就想看魔宴了，但因为那时我正处于看完另一部galgame的回味状态，满脑子都是那部galgame，所以就打算搞完这个项目后再来看魔宴。  
于是README里就有了这篇内容。  
