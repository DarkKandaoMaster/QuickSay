...
//2. 短语文本可以支持某些特定规则，于是实现高级输入：“在短语中插入标签，让QuickSay除了输入文字，还能按键、粘贴图片。”
...
void loadConfig(const QString & configPath){ //读取config.json到程序设置。如果config.json不存在，那么读取默认设置，同时写入默认设置到config.json
    ...
    if(file.open( QIODevice::ReadOnly | QIODevice::Text )){ //如果config.json成功打开（读模式）
        ...
        if(doc.isObject()){ //确认文档是一个对象
            ...
            //兼容旧版本
            if(!config.contains("delay")) config["delay"]=100;//如果config里没有delay，那么默认高级输入间隔100毫秒
        ...
        config["delay"]=100;//默认高级输入间隔100毫秒
...
bool showTabDialog(QWidget & parent,const QString & title,QString & tabName,int & itemHeight){ //用于弹出一个同时包含“标签名称”和“短语项高度”两个输入框的对话框【【【【【待处理，比如改下该窗口的UI
...
<html>
  ...
  <body>
    <h2>什么是高级输入？</h2>
    <p>在短语中插入标签，让QuickSay除了输入文字，还能按键、粘贴图片。</p>
    ...
    <ol>
      <li>第一条为图片。</li>
      <li>等待1秒。微信发送图片需要时间，添加<code>&lt;sleep&gt;</code>可以手动指定等待<br>时间，等待图片发送完成后再执行下一次高级输入，防止文字与图<br>片顺序错乱。</li>
      <li>第二条为“输入结束”。</li>
    </ol>
    ...
    <ol>
      ...
      <li>每执行一次高级输入，会按照设置里的“高级输入间隔”停顿一<br>下。</li>
    </ol>
...
int main(int argc, char *argv[]){
    ...
    liebiao.setStyleSheet(phraseItemStyle(   tabBar.tabData(tabBar.currentIndex()).toInt()   ));//程序启动时根据当前选中标签取出并应用短语项高度和内边距（注意程序启动时默认选中的是第一个标签。这是因为loadTabFromJson是在空标签栏里一个个新建标签的，所以Qt会选中第一个标签）
    ...
    QObject::connect(&tabBar,&QTabBar::currentChanged,
                     [&](int index){ //index是新选中标签的索引
                         if( index>=0 && index<tabBar.count() ){ //如果索引有效
                             liebiao.setStyleSheet(phraseItemStyle(   tabBar.tabData(index).toInt()   ));//根据当前选中标签取出并应用短语项高度和内边距
