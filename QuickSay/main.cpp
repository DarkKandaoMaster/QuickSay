//更新内容：
//1.修改了一下默认语录，删减了一些不重要的内容
//2.右键标签时，把“在当前标签后添加标签”这个选项移动到第一位
//3.把设置里的“列表滚动速度”改名为“滚动条滚动速度”
//4.可以对不同标签设置不同短语项高度了
//5.为列表项设置鼠标悬停时的提示文字。这样用户就可以通过鼠标悬停查看完整语录了
...
void loadConfig(const QString & configPath){ //读取config.json到程序设置。如果config.json不存在，那么读取默认设置，同时写入默认设置到config.json
    ...
    if(file.open( QIODevice::ReadOnly | QIODevice::Text )){ //如果config.json成功打开（读模式）
        ...
        if(doc.isObject()){ //确认文档是一个对象
            ...
            if(!config.contains("default_item_height")) config["default_item_height"]=40;//如果config里没有default_item_height，那么默认语录项高度40
        ...
        config["default_item_height"]=40;//默认语录项高度40
...
void updateItemDisplay(QListWidgetItem * it){ //更新对应列表项的显示，如果对应的备注不是空字符串，那么显示备注；否则显示语录
    ...
    it->setToolTip(   remark.isEmpty()?text:(remark+"\n────────────────────────\n"+text)   );//为列表项设置鼠标悬停时的提示文字。这样用户就可以通过鼠标悬停查看完整语录了
        ...
        if(doc.isArray()){ //确认文档是一个数组
            ...
            for(auto value:jsonArray){ //遍历数组中的每个元素
                if(value.isObject()){ //确认元素是对象
                    ...
                    QListWidgetItem * it=new QListWidgetItem();//new一个列表项对象，并把它的地址给it。不直接liebiao.addItem(text)是因为这样就用不了Qt::UserRole了；使用new是因为这样它就不会在当前函数结束时被销毁
...
int main(int argc, char *argv[]){
    ...
    QListWidget::item{                              /*因为语录字体颜色和备注字体颜色不同，所以不能在这里设置字体颜色，不然在这里设置的字体颜色会覆盖在其他地方设置的字体颜色*/
        ...
        padding: 12px 16px;                         /*列表项内边距：上下12像素，左右16像素*//*【【【注：想修改语录项高度在这里也可以修改】】】*/
    ...
    liebiao.setUniformItemSizes(true);//告诉视图所有列表项的高度都相同，这样才能正常自动折行。同时这会跳过逐个计算列表项的高度的过程，提高程序性能
    ...
    formLayout->addRow("默认语录项高度：",&itemHeightSpin);//在表单布局中添加一行，左边是标签“默认语录项高度：”，右边是数字输入框itemHeightSpin
    //如果用户修改了默认语录项高度
