...
//4.可以对不同标签设置不同短语项高度了
//5.为列表项设置鼠标悬停时的提示文字。这样用户就可以通过鼠标悬停查看完整短语了
...
void rebuildItemHotkeys(QListWidget & liebiao,QVector<QHotkey *> & itemHotkeys,QApplication * a){ //先禁用当前已注册的QHotkey *对象，然后遍历列表中的所有项，为它们注册快捷键
    ...
    for(int i=0;i<liebiao.count();i++){ //遍历列表中的所有项
        ...
        if(!hkStr.isEmpty()){ //如果hkStr不为空字符串
            ...
            //设置按下快捷键后会怎样，即输出该列表项里的短语
            QObject::connect(hk,&QHotkey::activated,
                             [it](){
                                 QString text=it->data(Qt::UserRole).toString();//获取该列表项里的短语
#ifdef _WIN32
...
int main(int argc, char *argv[]){
    ...
    QListWidget::item{                              /*因为短语字体颜色和备注字体颜色不同，所以不能在这里设置字体颜色，不然在这里设置的字体颜色会覆盖在其他地方设置的字体颜色*/
        ...
        padding: 12px 16px;                         /*列表项内边距：上下12像素，左右16像素*//*【【【注：想修改短语项高度在这里也可以修改】】】*/
    ...
    QObject::connect(&ctrlvCheck,&QCheckBox::toggled,
                     [&](bool checked){ //checked表示复选框的新状态，true表示勾选，false表示未勾选
                         if(checked){ //如果复选框勾上了
                             config["ctrlv"]=true;
                             clipboardCheck.setChecked(true);//同时也勾选输入时也将短语复制到剪贴板 //勾选clipboard复选框，同时触发信号函数改变config
                             saveConfig(configPath);
                         }
                         else{ //如果复选框取消勾选
                             config["ctrlv"]=false;
