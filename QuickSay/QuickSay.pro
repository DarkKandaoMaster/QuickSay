...
LIBS += -ltaskschd -lole32 -loleaut32 -ladvapi32 -lshell32 -lwtsapi32 -luuid #以管理员权限开机自启要用到计划任务、SID和UAC相关库；高级输入停止机制用wtsapi32监听锁屏、注销和会话断开；备份导入后通过Explorer重启需要Shell COM接口的GUID
